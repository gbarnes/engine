...
namespace Dawn
{
	...
	// Static definitions.
	std::mutex GfxResourceStateTracker::ms_GlobalMutex;
	bool GfxResourceStateTracker::ms_IsLocked = false;
	GfxResourceStateTracker::ResourceStateMap GfxResourceStateTracker::ms_GlobalResourceState;

	GfxResourceStateTracker::GfxResourceStateTracker()
	{}

	GfxResourceStateTracker::~GfxResourceStateTracker()
	{}

	void GfxResourceStateTracker::ResourceBarrier(const D3D12_RESOURCE_BARRIER& barrier)
	{
	...
	void GfxResourceStateTracker::TransitionResource(ID3D12Resource* resource, D3D12_RESOURCE_STATES stateAfter, UINT subResource)
	{
		if (resource)
		{
	...
	void GfxResourceStateTracker::TransitionResource(const GfxResource& resource, D3D12_RESOURCE_STATES stateAfter, UINT subResource)
	{
		TransitionResource(resource.GetD3D12Resource().Get(), stateAfter, subResource);
	}

	void GfxResourceStateTracker::UAVBarrier(const GfxResource* resource)
	{
	...
	void GfxResourceStateTracker::AliasBarrier(const GfxResource* resourceBefore, const GfxResource* resourceAfter)
	{
	...
	void GfxResourceStateTracker::FlushResourceBarriers(GfxCmdList& commandList)
	{
	...
	uint32_t GfxResourceStateTracker::FlushPendingResourceBarriers(GfxCmdList& commandList)
	{
		assert(ms_IsLocked);
	...
	void GfxResourceStateTracker::CommitFinalResourceStates()
	{
		assert(ms_IsLocked);
	...
	void GfxResourceStateTracker::Reset()
	{
	...
	void GfxResourceStateTracker::Lock()
	{
		ms_GlobalMutex.lock();
		ms_IsLocked = true;
	}

	void GfxResourceStateTracker::Unlock()
	{
		ms_GlobalMutex.unlock();
		ms_IsLocked = false;
	}

	void GfxResourceStateTracker::AddGlobalResourceState(ID3D12Resource* resource, D3D12_RESOURCE_STATES state)
	{
		if (resource != nullptr)
		{
	...
	void GfxResourceStateTracker::RemoveGlobalResourceState(ID3D12Resource* resource)
	{
		if (resource != nullptr)
		{
			std::lock_guard<std::mutex> lock(ms_GlobalMutex);
