...
namespace Dawn
{
	GfxTexture::GfxTexture(TextureUsage textureUsage, const std::wstring& name)
		: GfxResource(name)
		, m_TextureUsage(textureUsage)
	{
	}

	GfxTexture::GfxTexture(const D3D12_RESOURCE_DESC& resourceDesc,
		const D3D12_CLEAR_VALUE* clearValue,
		TextureUsage textureUsage,
		const std::wstring& name)
		: GfxResource(resourceDesc, clearValue, name)
		, m_TextureUsage(textureUsage)
	{
		CreateViews();
	}

	GfxTexture::GfxTexture(Microsoft::WRL::ComPtr<ID3D12Resource> resource,
		TextureUsage textureUsage,
		const std::wstring& name)
		: GfxResource(resource, name)
		, m_TextureUsage(textureUsage)
	{
		CreateViews();
	}

	GfxTexture::GfxTexture(const GfxTexture& copy)
		: GfxResource(copy)
	{
		CreateViews();
	}

	GfxTexture::GfxTexture(GfxTexture&& copy)
		: GfxResource(copy)
	{
		CreateViews();
	}

	GfxTexture& GfxTexture::operator=(const GfxTexture& other)
	{
		GfxResource::operator=(other);
	...
	void GfxTexture::Resize(uint32_t width, uint32_t height, uint32_t depthOrArraySize)
	{
		// Resource can't be resized if it was never created in the first place.
		if (m_d3d12Resource)
		{
			GfxResourceStateTracker::RemoveGlobalResourceState(m_d3d12Resource.Get());
	...
	void GfxTexture::CreateViews()
	{
		if (m_d3d12Resource)
		{
			...
			{
				m_RenderTargetView = GfxBackend::AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
			...
			{
				m_DepthStencilView = GfxBackend::AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
	...
	D3D12_CPU_DESCRIPTOR_HANDLE GfxTexture::GetShaderResourceView(const D3D12_SHADER_RESOURCE_VIEW_DESC* srvDesc) const
	{
		std::size_t hash = 0;
		if (srvDesc)
		{
	...
	D3D12_CPU_DESCRIPTOR_HANDLE GfxTexture::GetUnorderedAccessView(const D3D12_UNORDERED_ACCESS_VIEW_DESC* uavDesc) const
	{
		std::size_t hash = 0;
		if (uavDesc)
		{
	...
	D3D12_CPU_DESCRIPTOR_HANDLE GfxTexture::GetRenderTargetView() const
	{
		return m_RenderTargetView.GetDescriptorHandle();
	}

	D3D12_CPU_DESCRIPTOR_HANDLE GfxTexture::GetDepthStencilView() const
	{
		return m_DepthStencilView.GetDescriptorHandle();
	}

	bool GfxTexture::IsUAVCompatibleFormat(DXGI_FORMAT format)
	{
		switch (format)
		{
	...
	bool GfxTexture::IsSRGBFormat(DXGI_FORMAT format)
	{
		switch (format)
		{
	...
	bool GfxTexture::IsBGRFormat(DXGI_FORMAT format)
	{
		switch (format)
		{
	...
	bool GfxTexture::IsDepthFormat(DXGI_FORMAT format)
	{
		switch (format)
		{
	...
	DXGI_FORMAT GfxTexture::GetTypelessFormat(DXGI_FORMAT format)
	{
