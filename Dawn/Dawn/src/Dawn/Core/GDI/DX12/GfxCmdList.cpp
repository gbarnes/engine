...
namespace Dawn 
{
	std::map<std::wstring, ID3D12Resource* > GfxCmdList::ms_TextureCache;
	std::mutex GfxCmdList::ms_TextureCacheMutex;
		...
		: m_d3d12CommandListType(type)
	{
		...
		m_UploadBuffer = std::make_unique<GfxUploadBuffer>();

		m_ResourceStateTracker = std::make_unique<GfxResourceStateTracker>();

		for (int i = 0; i < D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES; ++i)
		{
			m_DynamicDescriptorHeap[i] = std::make_unique<GfxDynamicDescriptorHeap>(static_cast<D3D12_DESCRIPTOR_HEAP_TYPE>(i));
			m_DescriptorHeaps[i] = nullptr;
		}
	}

	GfxCmdList::~GfxCmdList()
	{}

	void GfxCmdList::TransitionBarrier(const GfxResource& resource, D3D12_RESOURCE_STATES stateAfter, UINT subResource, bool flushBarriers)
	{
	...
	void GfxCmdList::UAVBarrier(const GfxResource& resource, bool flushBarriers)
	{
		auto d3d12Resource = resource.GetD3D12Resource();
	...
	void GfxCmdList::AliasingBarrier(const GfxResource& beforeResource, const GfxResource& afterResource, bool flushBarriers)
	{
	...
	void GfxCmdList::FlushResourceBarriers()
	{
		m_ResourceStateTracker->FlushResourceBarriers(*this);
	}

	void GfxCmdList::CopyResource(GfxResource& dstRes, const GfxResource& srcRes)
	{
	...
	void GfxCmdList::ResolveSubresource(GfxResource& dstRes, const GfxResource& srcRes, uint32_t dstSubresource, uint32_t srcSubresource)
	{
	...
	void GfxCmdList::CopyBuffer(GfxBuffer& buffer, size_t numElements, size_t elementSize, const void* bufferData, D3D12_RESOURCE_FLAGS flags)
	{
		...
		else
		{
			...
			GfxResourceStateTracker::AddGlobalResourceState(d3d12Resource.Get(), D3D12_RESOURCE_STATE_COMMON);
	...
	void GfxCmdList::CopyVertexBuffer(GfxVertexBuffer& vertexBuffer, size_t numVertices, size_t vertexStride, const void* vertexBufferData)
	{
		CopyBuffer(vertexBuffer, numVertices, vertexStride, vertexBufferData);
	}

	void GfxCmdList::CopyIndexBuffer(GfxIndexBuffer& indexBuffer, size_t numIndicies, DXGI_FORMAT indexFormat, const void* indexBufferData)
	{
		size_t indexSizeInBytes = indexFormat == DXGI_FORMAT_R16_UINT ? 2 : 4;
		CopyBuffer(indexBuffer, numIndicies, indexSizeInBytes, indexBufferData);
	}

	void GfxCmdList::CopyByteAddressBuffer(GfxByteAddressBuffer& byteAddressBuffer, size_t bufferSize, const void* bufferData)
	{
		CopyBuffer(byteAddressBuffer, 1, bufferSize, bufferData, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	}

	void GfxCmdList::CopyStructuredBuffer(GfxStructuredBuffer& structuredBuffer, size_t numElements, size_t elementSize, const void* bufferData)
	{
		CopyBuffer(structuredBuffer, numElements, elementSize, bufferData, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	}

	void GfxCmdList::SetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY primitiveTopology)
	{
	...
	void GfxCmdList::LoadTextureView(GfxTexture& texture, Image* image, TextureUsage textureUsage)
	{
		...
		{*/
			...
			//case DirectX::TEX_DIMENSION_TEXTURE2D:
				textureDesc = CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT::DXGI_FORMAT_R8G8B8A8_UNORM,
			...
			GfxResourceStateTracker::AddGlobalResourceState(textureResource.Get(), D3D12_RESOURCE_STATE_COMMON);
	...
	void GfxCmdList::GenerateMips(GfxTexture& texture)
	{
		if (m_d3d12CommandListType == D3D12_COMMAND_LIST_TYPE_COPY)
		{
			...
			{
				m_ComputeCommandList = GfxBackend::GetQueue(D3D12_COMMAND_LIST_TYPE_COMPUTE)->GetCommandList();
			}
		...
		if (GfxTexture::IsUAVCompatibleFormat(d3d12ResourceDesc.Format))
		{
			GenerateMips_UAV(texture);
		}
		else if (GfxTexture::IsBGRFormat(d3d12ResourceDesc.Format))
		{
			GenerateMips_BGR(texture);
		}
		else if (GfxTexture::IsSRGBFormat(d3d12ResourceDesc.Format))
		{
	...
	void GfxCmdList::GenerateMips_UAV(GfxTexture& texture)
	{
		...
		GfxTexture stagingTexture(stagingResource);
		...
		if ((resourceDesc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS) == 0)
		{
			...
			GfxResourceStateTracker::AddGlobalResourceState(stagingResource.Get(), D3D12_RESOURCE_STATE_COPY_DEST);
	...
	void GfxCmdList::GenerateMips_BGR(GfxTexture& texture)
	{
		...
		GfxResourceStateTracker::AddGlobalResourceState(resourceCopy.Get(), D3D12_RESOURCE_STATE_COMMON);

		GfxTexture copyTexture(resourceCopy);
		...
		GfxResourceStateTracker::AddGlobalResourceState(aliasCopy.Get(), D3D12_RESOURCE_STATE_COMMON);

		// Copy the original texture to the aliased texture.
		GfxTexture aliasTexture(aliasCopy);
		AliasingBarrier(GfxTexture(), aliasTexture); // There is no "before" texture. 
	...
	void GfxCmdList::PanoToCubemap(GfxTexture& cubemapTexture, const GfxTexture& panoTexture)
	{
		if (m_d3d12CommandListType == D3D12_COMMAND_LIST_TYPE_COPY)
		{
			if (!m_ComputeCommandList)
		...
		for (uint32_t mipSlice = 0; mipSlice < cubemapDesc.MipLevels; )
		{
			...
			//SetCompute32BitConstants(PanoToCubemapCB, panoToCubemapCB);

			//SetShaderResourceView(GfxPanoToCubemapRS::SrcTexture, 0, panoTexture, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);

			for (uint32_t mip = 0; mip < numMips; ++mip)
			{
				uavDesc.Texture2DArray.MipSlice = mipSlice + mip;
				//SetUnorderedAccessView(GfxPanoToCubemapRS::DstMips, mip, stagingTexture, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, 0, 0, &uavDesc);
			}
	...
	void GfxCmdList::ClearTexture(const GfxTexture& texture, const float clearColor[4])
	{
	...
	void GfxCmdList::ClearDepthStencilTexture(const GfxTexture& texture, D3D12_CLEAR_FLAGS clearFlags, float depth, uint8_t stencil)
	{
	...
	void GfxCmdList::SetGraphicsDynamicConstantBuffer(uint32_t rootParameterIndex, size_t sizeInBytes, const void* bufferData)
	{
	...
	void GfxCmdList::SetGraphics32BitConstants(uint32_t rootParameterIndex, uint32_t numConstants, const void* constants)
	{
		m_d3d12CommandList->SetGraphicsRoot32BitConstants(rootParameterIndex, numConstants, constants, 0);
	}

	void GfxCmdList::SetCompute32BitConstants(uint32_t rootParameterIndex, uint32_t numConstants, const void* constants)
	{
		m_d3d12CommandList->SetComputeRoot32BitConstants(rootParameterIndex, numConstants, constants, 0);
	}

	void GfxCmdList::SetVertexBuffer(uint32_t slot, const GfxVertexBuffer& vertexBuffer)
	{
	...
	void GfxCmdList::SetDynamicVertexBuffer(uint32_t slot, size_t numVertices, size_t vertexSize, const void* vertexBufferData)
	{
	...
	void GfxCmdList::SetIndexBuffer(const GfxIndexBuffer& indexBuffer)
	{
	...
	void GfxCmdList::SetDynamicIndexBuffer(size_t numIndicies, DXGI_FORMAT indexFormat, const void* indexBufferData)
	{
		size_t indexSizeInBytes = indexFormat == DXGI_FORMAT_R16_UINT ? 2 : 4;
	...
	void GfxCmdList::SetGraphicsDynamicStructuredBuffer(uint32_t slot, size_t numElements, size_t elementSize, const void* bufferData)
	{
		size_t bufferSize = numElements * elementSize;
	...
	void GfxCmdList::SetViewport(const D3D12_VIEWPORT& viewport)
	{
		SetViewports({ viewport });
	}

	void GfxCmdList::SetViewports(const std::vector<D3D12_VIEWPORT>& viewports)
	{
	...
	void GfxCmdList::SetScissorRect(const D3D12_RECT& scissorRect)
	{
		SetScissorRects({ scissorRect });
	}

	void GfxCmdList::SetScissorRects(const std::vector<D3D12_RECT>& scissorRects)
	{
	...
	void GfxCmdList::SetPipelineState(Microsoft::WRL::ComPtr<ID3D12PipelineState> pipelineState)
	{
	...
	void GfxCmdList::SetGraphicsRootSignature(const GfxRootSignature& rootSignature)
	{
	...
	void GfxCmdList::SetComputeRootSignature(const GfxRootSignature& rootSignature)
	{
		auto d3d12RootSignature = rootSignature.GetRootSignature().Get();
		if (m_RootSignature != d3d12RootSignature)
		{
	...
	void GfxCmdList::SetShaderResourceView(uint32_t rootParameterIndex,
	...
	void GfxCmdList::SetUnorderedAccessView(uint32_t rootParameterIndex,
	...
	void GfxCmdList::SetRenderTarget(const GfxRenderTarget& renderTarget)
	{
	...
	void GfxCmdList::Draw(uint32_t vertexCount, uint32_t instanceCount, uint32_t startVertex, uint32_t startInstance)
	{
		FlushResourceBarriers();
	...
	void GfxCmdList::DrawIndexed(uint32_t indexCount, uint32_t instanceCount, uint32_t startIndex, int32_t baseVertex, uint32_t startInstance)
	{
		FlushResourceBarriers();
	...
	void GfxCmdList::Dispatch(uint32_t numGroupsX, uint32_t numGroupsY, uint32_t numGroupsZ)
	{
		FlushResourceBarriers();
	...
	bool GfxCmdList::Close(GfxCmdList& pendingCommandList)
	{
	...
	void GfxCmdList::TrackObject(Microsoft::WRL::ComPtr<ID3D12Object> object)
	{
		m_TrackedObjects.push_back(object);
	}

	void GfxCmdList::TrackResource(const GfxResource& res)
	{
		TrackObject(res.GetD3D12Resource());
	}

	void GfxCmdList::ReleaseTrackedObjects()
	{
		m_TrackedObjects.clear();
	}

	void GfxCmdList::SetDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE heapType, ID3D12DescriptorHeap* heap)
	{
	...
	void GfxCmdList::BindDescriptorHeaps()
	{
