...
namespace Dawn
{
	...
	ResourceSystem::ResourceSystem(std::string InPath, std::vector<std::string> InFilter)
		...
		, Filters(InFilter)
	{
		Locator::Add(FileSystemId, this);
	}

	ResourceSystem::~ResourceSystem()
	{
		Locator::Remove(FileSystemId);
	}
	...
	bool ResourceSystem::BuildDatabase()
	{
		...
		for (const auto& entry : it)
		{
			...
			if (entry.is_regular_file() && IsValidFilter(extension))
			{
				...
				DWN_INFO("Added {0} ({1}) to file database...", metaData.Name, metaData.Id);
			}
		}
	...
	u128 ResourceSystem::ResourceAmount(ResourceType InType)
	{
